...
public class FrmSearchCustomer extends JDialog{
	...
	public FrmSearchCustomer(JFrame OwnerForm){
		...
		String StrListItem[]={"Customer ID","Company Name","Contact Name"};
		...
		public void actionPerformed(ActionEvent e){
			...
			if(srcObj=="search"){
				if(JTFSearchFor.getText().equals("")){
					...
					FrmCustomer.reloadRecord("SELECT * FROM tblCustomer WHERE " + JCSearchIn.getSelectedItem().toString().replaceAll(" ", "") + " LIKE '%" + JTFSearchFor.getText() + "%' ORDER BY CompanyName ASC");
					dispose();
				}
