...
public class FrmSearchWarehouse extends JDialog{
	...
	public FrmSearchWarehouse(JFrame OwnerForm){
		super(OwnerForm,true);
	    setTitle("Search Warehouse");

		
		String StrListItem[]={"Warehouse ID","Warehouse Name","Contact Person"};
		...
		public void actionPerformed(ActionEvent e){
			...
			if(srcObj=="search"){
				if(JTFSearchFor.getText().equals("")){
					...
					FrmWarehouse.reloadRecord("SELECT * FROM tblWarehouse WHERE " + JCSearchIn.getSelectedItem().toString().replaceAll(" ", "") + " LIKE '%" + JTFSearchFor.getText() + "%' ORDER BY WarehouseName ASC");
					dispose();
				}
